								...
								<div class="app-toolbar-wrapper d-flex flex-stack flex-wrap gap-4 w-100 mt-3 mb-2">
									...
									<div class="d-flex align-items-center gap-2 gap-lg-3">
										<div class="card-title fs-3 fw-bold">
											...
											<button class="btn btn-primary ml-2" type="button" id="btn_simpan" onclick="update_data()"><b><i><i class="ki-outline ki-send"></i></i> Update</b></button>
										</div>
									</div>
								</div>
							</div>
							<div class="card-body">
								<form id="formData" name="form_update">
									<?php foreach ($detail as $row) : ?>
										<input type="hidden" hidden id="id" name="id" value="<?= $row->id_goal; ?>">
										<input type="hidden" hidden id="incr" name="incr" value="<?= $row->incr; ?>">
										<div class="form-group">
											<label for="nama">Nama Goal</label>
											<input type="text" value="<?= $row->nama_goal; ?>" data-parsley-errors-container=".err_nama" data-parsley-required="true" required="" class="form-control col-8" id="nama" name="nama" placeholder="Masukkan Nama">
											<span class="text-danger err_nama"></span>
										</div>
										<div class="form-group">
											<label for="ket">Keteranggan</label>
											<textarea rows="3" name="ket" id="ket" data-parsley-errors-container=".err_ket" data-parsley-required="true" required="" class="form-control col-8" placeholder="Masukkan keteranggan"><?= $row->keterangan; ?></textarea>
